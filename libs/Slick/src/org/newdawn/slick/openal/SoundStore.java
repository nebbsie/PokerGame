...
/**
 * Responsible for holding and playing the Music used in the game.
 * 
 * @author Kevin Glass
...
public class SoundStore {
	...
	/** The map of references to IDs of previously loaded Music */
     ...
     * 
     * @return True if we're loading deferred Music
     */
	...
	public Audio getMOD(String ref, InputStream in) throws IOException {
		...
		if (!inited) {
			throw new RuntimeException("Can't load Music until SoundStore is init(). Use the container init() method.");
		}
		if (deferred) {
		...
		if (music) {
			if (mod != null) {
				try {
					...
					Log.error("Error with OpenGL MOD PlayerOld on this this platform");
					Log.error(e);
					mod = null;
				}
			}
			if (stream != null) {
				try {
					...
					Log.error("Error with OpenGL Streaming PlayerOld on this this platform");
					Log.error(e);
					mod = null;
				}
			}
